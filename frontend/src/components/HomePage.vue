      ...
      <v-card width="35%" color="#EFEBE9" elevation="7">
        <v-form ref="signInForm" valid="validForm">
          ...
          <v-col cols="12" md="12" sm="12" class="py-0">
            <v-text-field v-model="user.password" placeholder="Enter Password" variant="outlined" label="Password"
              :append-inner-icon="visible ? 'mdi-eye-off' : 'mdi-eye'" :type="visible ? 'text' : 'password'"
              @click:append-inner="visible = !visible" :rules="[rules.required, rules.maxlength]" required />
          </v-col>
          ...
          <v-row class="d-flex align-center">
            ...
            <v-col class="mb-1 pl-0" cols="6">
              <v-btn variant="outlined" color="brown" class="ml-0" size="small" @click="dialog = true">
                Register Now
              </v-btn>
            </v-col>
          </v-row>
